...
function setCardStrings() {
  ...
  for (var i = 0; i < players.length; i++) {
    ...
    var cString = players[i].cardString;
...
function createDeck() {
  ...
  for (var suitIDX = 0; suitIDX < suits.length; suitIDX++) {
    for (var valueIDX = 0; valueIDX < values.length; valueIDX++) {
      var card = { suit: suits[suitIDX], value: values[valueIDX] };
      newDeck.push(card);
    }
  }
...
function shuffleDeck() {
  for (var cardIDX = 0; cardIDX < deck.length; cardIDX++) {
    var randomIDX = Math.trunc(Math.random() * deck.length); //random # between 0 and 51
    var tempIDX = deck[randomIDX];
...
function setDOMVisibility() {
  ...
  } else if (gameFlags.playerWon || gameFlags.dealerWon) {
    //not inSession
...
function checkForEndOfGame() {
  ...
  if (!gameFlags.inSession) {
    ...
    while (players[0].score < players[1].score && players[1].score <= 21 && //player
    players[0].score <= 16) {
      //dealer takes his last hit at 16
